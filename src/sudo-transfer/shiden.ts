...
const SHIDEN_ASSET_ID = '340282366920938463463374607431768211455';
const execute_fee = new BN(4000000);

export default async function index() {
  const senderKey = process.env.SUBSTRATE_MNEMONIC || '//Alice';
  const api = new SubstrateApi(endpoints.parachain.shiden, senderKey);
  ...
  const allExtrinsics = (await utils.readJson('./extrinsics-shiden.json')) as TransferItem[];
  const chunks = 100;

  const assetId = SHIDEN_ASSET_ID;
  ...
  // save as file
  await utils.saveAsJson(batchCalls, './dot-mint-batch-shiden.json');
